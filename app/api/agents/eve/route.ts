...
export async function GET() {
  ...
  const scanMsg = await client.messages.create({
    ...
    messages: [
      {
        role: "user",
        content: `You are Eve, Eden's maintenance agent. Scan these recent build failures and identify maintenance tasks.
...
export async function POST(req: NextRequest) {
  ...
  // Create build tagged as Eve (maintenance)
  const build = await prisma.agentBuild.create({
    data: {
      userId: session.user.id,
      request: `[eden-eve] ${description}`,
      status: "running",
    },
  });

  // Eve uses Haiku only — lightweight maintenance tasks
  const message = await client.messages.create({
    ...
    messages: [
      {
        role: "user",
        content: `You are Eve, Eden's maintenance agent. Break this maintenance task into 2-4 small tasks: "${description}"
...
Return ONLY the JSON array.`,
  ...
  // Trigger architect for the maintenance build
  try {
    ...
    }).catch((err) => console.error("[eve] Architect trigger failed:", err));
  } catch {
    // Non-blocking
  }
