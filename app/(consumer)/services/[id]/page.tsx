...
export default async function ServiceDetailPage({
  ...
  const consumerAvailabilityDetail = businessFrozen
    ...
    : pipelineStatusOverride === "published" || pipelineSnapshot?.status === "published"
      ? "This service is live in Eden discovery and can be run through the Eden Leafs wallet flow right now."
      : pipelineStatusOverride === "ready" || pipelineSnapshot?.status === "ready"
    ...
    <DetailPlaceholderPanel
      ...
      note={
        businessFrozen
          ...
          : `This route uses the current service record, visible pricing, and wallet-aware usage flow. The price shown below is the exact Eden Leafs amount used for service runs, and ${edenLaunchLabels.noHiddenCheckout.toLowerCase()}`
      }
    >
          ...
          <div className="rounded-2xl border border-eden-edge bg-[linear-gradient(135deg,rgba(219,234,254,0.4),rgba(255,255,255,0.96))] p-4">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
              ...
              <div className="rounded-2xl border border-eden-edge bg-white p-3">
                ...
                <p className="mt-2 text-sm leading-6 text-eden-muted">
                  Service runs deduct Leafs from the wallet. Stripe only appears if the user explicitly chooses to top up.
                </p>
              </div>
              <div className="rounded-2xl border border-eden-edge bg-white p-3">
          ...
          <div className="rounded-2xl border border-eden-edge bg-white p-4">
            ...
            <div className="mt-4 space-y-3">
              <div className="rounded-2xl border border-eden-edge bg-eden-bg/60 p-3">
                ...
                <p className="mt-2 text-sm leading-6 text-eden-muted">
                  Eden shows whether the service is live and exactly what the Eden Leafs price is before anything runs.
                </p>
              </div>
              <div className="rounded-2xl border border-eden-edge bg-eden-bg/60 p-3">
              ...
              <div className="rounded-2xl border border-eden-edge bg-eden-bg/60 p-3">
                <p className="text-sm font-semibold text-eden-ink">3. Add Leafs only if needed</p>
                <p className="mt-2 text-sm leading-6 text-eden-muted">
                  Add Leafs through the wallet flow if your balance is low. {edenLaunchLabels.noHiddenCheckout}
                </p>
              </div>
              <div className="rounded-2xl border border-eden-edge bg-eden-bg/60 p-3">
                <p className="text-sm font-semibold text-eden-ink">4. Run the service</p>
                <p className="mt-2 text-sm leading-6 text-eden-muted">
                  A successful run deducts the visible Leafs amount, records usage, and updates the wallet history immediately.
                </p>
              </div>
            </div>
          </div>
