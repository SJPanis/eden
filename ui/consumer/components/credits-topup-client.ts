...
export async function startPaymentBackedCreditsTopUp(
  ...
  if (!response.ok || !payload.ok || !payload.checkoutUrl) {
    throw new Error(payload.error || "Unable to start the payment-backed Leafs top-up.");
  }
...
export async function confirmPaymentBackedCreditsTopUp(sessionId: string) {
  ...
  if (!response.ok || !payload.ok) {
    throw new Error(payload.error || "Unable to confirm the payment-backed Leafs top-up.");
  }
...
export function buildPaymentTopUpReceipt(
  ...
  const receiptDetail = payload.alreadyApplied
    ? payload.settlementSummary ??
      `Stripe already settled ${selectedPackage.title}. Eden Leafs were previously added to this wallet through the webhook-authoritative top-up flow.`
    : payload.settlementSummary ??
      `Stripe settlement confirmed for ${selectedPackage.title}. Eden Leafs were added through the webhook-authoritative top-up flow.`;
...
export function buildTopUpCancellationMessage(
  ...
  return {
    ...
    detail: `${selectedPackage.title} was not purchased. Eden Leafs were not added to the wallet.`,
  };
}
...
export function buildTopUpFailureMessage(
  ...
  return {
    ...
    title: "Top-up not settled",
      detail:
        message ??
      `${selectedPackage.title} did not settle successfully. No Eden Leafs were added.`,
  };
}
