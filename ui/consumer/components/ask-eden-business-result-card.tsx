  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <h4 className="text-sm font-semibold text-white">{business.name}</h4>
        <span
      ...
      <div className="mt-2 flex flex-wrap gap-1.5">
        {business.tags.map((tag) => (
          <span
            key={`${business.id}-${tag}`}
            className="rounded-full bg-white/[0.04] px-2 py-0.5 text-[10px] uppercase tracking-[0.08em] text-white/50"
          >
            {tag}
          </span>
        ))}
      </div>
      <p className="mt-3 text-xs leading-5 text-white/50">{business.summary}</p>
      <button
        type="button"
        onClick={(event) => {
